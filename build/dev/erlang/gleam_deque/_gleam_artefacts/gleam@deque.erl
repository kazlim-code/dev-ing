...
-opaque deque(EZW) :: {deque, list(EZW), list(EZW)}.
...
-spec from_list(list(EZZ)) -> deque(EZZ).
from_list(List) ->
    {deque, [], List}.
...
-spec to_list(deque(FAC)) -> list(FAC).
to_list(Deque) ->
...
-spec push_back(deque(FAJ), FAJ) -> deque(FAJ).
...
-spec push_front(deque(FAM), FAM) -> deque(FAM).
...
-spec pop_back(deque(FAP)) -> {ok, {FAP, deque(FAP)}} | {error, nil}.
pop_back(Deque) ->
    case Deque of
        {deque, [], []} ->
            {error, nil};
...
-spec pop_front(deque(FAU)) -> {ok, {FAU, deque(FAU)}} | {error, nil}.
pop_front(Deque) ->
    case Deque of
        {deque, [], []} ->
            {error, nil};
...
-spec reverse(deque(FAZ)) -> deque(FAZ).
reverse(Deque) ->
...
-spec is_logically_equal(deque(FBC), deque(FBC), fun((FBC, FBC) -> boolean())) -> boolean().
...
-spec is_equal(deque(FBK), deque(FBK)) -> boolean().
is_equal(A, B) ->
    check_equal(
        erlang:element(3, A),
        erlang:element(2, A),
