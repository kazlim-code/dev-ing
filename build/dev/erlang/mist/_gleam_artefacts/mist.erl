...
-opaque next(AAGV, AAGW) :: {continue,
        AAGV,
        gleam@option:option(gleam@erlang@process:selector(AAGW))} |
    normal_stop |
...
-opaque builder(AAGX, AAGY) :: {builder,
        integer(),
        fun((gleam@http@request:request(AAGX)) -> gleam@http@response:response(AAGY)),
...
-type websocket_message(AAGZ) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, AAGZ}.
...
-spec continue(AAHA) -> next(AAHA, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 58).
-spec with_selector(next(AAHE, AAHF), gleam@erlang@process:selector(AAHF)) -> next(AAHE, AAHF).
...
-spec convert_next(next(AAHT, AAHU)) -> mist@internal@next:next(AAHT, AAHU).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(AAIV)) -> gleam@http@response:response(AAIX))
) -> builder(AAIV, AAIX).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(AAJB, AAJC), integer()) -> builder(AAJB, AAJC).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), AAJH),
    integer(),
    gleam@http@response:response(AAJH)
) -> builder(mist@internal@http:connection(), AAJH).
...
-spec bind(builder(AAJT, AAJU), binary()) -> builder(AAJT, AAJU).
...
-spec with_ipv6(builder(AAJZ, AAKA)) -> builder(AAJZ, AAKA).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(AAKF, AAKG), binary(), binary()) -> builder(AAKF, AAKG).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(AAKV)
) -> {ok, websocket_message(AAKV)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((AALB, websocket_message(AALC), mist@internal@websocket:websocket_connection()) -> next(AALB, AALC)),
    fun((mist@internal@websocket:websocket_connection()) -> {AALB,
        gleam@option:option(gleam@erlang@process:selector(AALC))}),
    fun((AALB) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(AALQ)) -> {ok,
            gleam@otp@actor:initialised(AALS, AALQ, any())} |
        {error, binary()}),
    fun((AALS, AALQ, s_s_e_connection()) -> gleam@otp@actor:next(AALS, AALQ))
) -> gleam@http@response:response(response_data()).
