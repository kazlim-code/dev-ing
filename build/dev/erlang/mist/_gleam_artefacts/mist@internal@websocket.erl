...
-type valid_message(YSL) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, YSL}.

-type websocket_message(YSM) :: {valid, valid_message(YSM)} | invalid.
...
-type handler_message(YSN) :: {internal, gramps@websocket:frame()} | {user, YSN}.

-type websocket_state(YSO) :: {websocket_state,
        bitstring(),
        YSO,
...
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(YTU))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(YTU))).
...
-spec apply_frames(
    ...
    fun((YTK, handler_message(YTL), websocket_connection()) -> mist@internal@next:next(YTK, YTL)),
    websocket_connection(),
    mist@internal@next:next(YTK, websocket_message(YTL)),
    fun((YTK) -> nil)
) -> mist@internal@next:next(YTK, websocket_message(YTL)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {YSX,
        gleam@option:option(gleam@erlang@process:selector(YSY))}),
    fun((YSX) -> nil),
    fun((YSX, handler_message(YSY), websocket_connection()) -> mist@internal@next:next(YSX, YSY)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(websocket_message(YSY)))} |
    {error, nil}.
