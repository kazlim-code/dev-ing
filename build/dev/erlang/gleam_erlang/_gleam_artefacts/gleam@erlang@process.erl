...
-opaque subject(FFY) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(FFY)}.

-type name(FFZ) :: any() | {gleam_phantom, FFZ}.

-type do_not_leak() :: any().

-type selector(FGA) :: any() | {gleam_phantom, FGA}.
...
-spec 'receive'(subject(FGY), integer()) -> {ok, FGY} | {error, nil}.
...
-spec receive_forever(subject(FHG)) -> FHG.
...
-spec selector_receive(selector(FHK), integer()) -> {ok, FHK} | {error, nil}.
...
-spec selector_receive_forever(selector(FHO)) -> FHO.
...
-spec map_selector(selector(FHQ), fun((FHQ) -> FHS)) -> selector(FHS).
map_selector(A, B) ->
...
-spec merge_selector(selector(FHU), selector(FHU)) -> selector(FHU).
merge_selector(A, B) ->
...
-spec select_map(selector(FIF), subject(FIH), fun((FIH) -> FIF)) -> selector(FIF).
...
-spec select(selector(FIB), subject(FIB)) -> selector(FIB).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> FIP)
) -> selector(FIP).
...
-spec select_other(selector(FIT), fun((gleam@dynamic:dynamic_()) -> FIT)) -> selector(FIT).
...
-spec deselect(selector(FIK), subject(any())) -> selector(FIK).
...
-spec select_specific_monitor(selector(FJF), monitor(), fun((down()) -> FJF)) -> selector(FJF).
...
-spec select_monitors(selector(FJI), fun((down()) -> FJI)) -> selector(FJI).
...
-spec select_trapped_exits(selector(FHY), fun((exit_message()) -> FHY)) -> selector(FHY).
...
-spec deselect_specific_monitor(selector(FJL), monitor()) -> selector(FJL).
...
-spec send_after(subject(FKH), integer(), FKH) -> timer().
...
-spec perform_call(
    subject(FJO),
    fun((subject(FJQ)) -> FJO),
    fun((selector(FJQ)) -> {ok, FJQ} | {error, nil})
) -> FJQ.
...
-spec call(subject(FJV), integer(), fun((subject(FJX)) -> FJV)) -> FJX.
...
-spec call_forever(subject(FJZ), fun((subject(FKB)) -> FJZ)) -> FKB.
