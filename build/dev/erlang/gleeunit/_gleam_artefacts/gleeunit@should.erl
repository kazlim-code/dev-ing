...
-spec be_ok({ok, KAG} | {error, any()}) -> KAG.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, KAL}) -> KAL.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(KAO)) -> KAO.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
