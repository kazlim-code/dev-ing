...
-spec one_of(list(KEK)) -> fun((KEK) -> {ok, KEK} | {error, snag:snag()}).
one_of(Allowed) ->
...
-spec none_of(list(KEN)) -> fun((KEN) -> {ok, KEN} | {error, snag:snag()}).
none_of(Disallowed) ->
...
-spec each(fun((KEQ) -> {ok, KEQ} | {error, snag:snag()})) -> fun((list(KEQ)) -> {ok,
        list(KEQ)} |
    {error, snag:snag()}).
each(Constraint) ->
