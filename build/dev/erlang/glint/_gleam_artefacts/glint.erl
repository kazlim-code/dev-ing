...
-opaque glint(KKU) :: {glint, config(), command_node(KKU)}.

-type args_count() :: {eq_args, integer()} | {min_args, integer()}.

-opaque command(KKV) :: {command,
        fun((named_args(), list(binary()), flags()) -> KKV),
        flags(),
        binary(),
        gleam@option:option(args_count()),
        list(binary())}.

-type internal_command(KKW) :: {internal_command,
        fun((named_args(), list(binary()), flags()) -> KKW),
        flags(),
        gleam@option:option(args_count()),
        list(binary())}.
...
-type command_node(KKX) :: {command_node,
        gleam@option:option(internal_command(KKX)),
        gleam@dict:dict(binary(), command_node(KKX)),
        flags(),
        binary()}.

-type out(KKY) :: {out, KKY} | {help, binary()}.
...
-opaque flag(KKZ) :: {flag,
        binary(),
        binary(),
        fun((binary()) -> {ok, KKZ} | {error, snag:snag()}),
        fun((flag_internals(KKZ)) -> value()),
        fun((flags(), binary()) -> {ok, KKZ} | {error, snag:snag()}),
        gleam@option:option(KKZ)}.

-type flag_internals(KLA) :: {flag_internals,
        gleam@option:option(KLA),
        fun((binary()) -> {ok, KLA} | {error, snag:snag()})}.
...
-spec pretty_help(glint(KLH), pretty_help()) -> glint(KLH).
...
-spec with_name(glint(KLK), binary()) -> glint(KLK).
...
-spec without_exit(glint(KLN)) -> glint(KLN).
without_exit(Glint) ->
    {glint,
        begin
            _record = erlang:element(2, Glint),
...
-spec as_module(glint(KLQ)) -> glint(KLQ).
as_module(Glint) ->
    {glint,
        begin
            _record = erlang:element(2, Glint),
...
-spec with_indent_width(glint(KLT), integer()) -> glint(KLT).
...
-spec with_max_output_width(glint(KLW), integer()) -> glint(KLW).
...
-spec with_min_first_column_width(glint(KLZ), integer()) -> glint(KLZ).
...
-spec with_column_gap(glint(KMC), integer()) -> glint(KMC).
...
-spec global_help(glint(KML), binary()) -> glint(KML).
...
-spec map_command(command(KNA), fun((KNA) -> KNC)) -> command(KNC).
...
-spec command_help(binary(), fun(() -> command(KNE))) -> command(KNE).
command_help(Desc, F) ->
    _record = F(),
    {command,
        erlang:element(2, _record),
...
-spec unnamed_args(args_count(), fun(() -> command(KNH))) -> command(KNH).
...
-spec named_arg(
    binary(),
    fun((fun((named_args()) -> binary())) -> command(KNK))
) -> command(KNK).
named_arg(Name, F) ->
    Cmd = begin
        F(
...
-spec new_builder(
    binary(),
    fun((flag_internals(KPY)) -> value()),
    fun((flags(), binary()) -> {ok, KPY} | {error, snag:snag()}),
    fun((binary()) -> {ok, KPY} | {error, snag:snag()})
) -> flag(KPY).
...
-spec wrap_with_constraint(
    fun((binary()) -> {ok, KQK} | {error, snag:snag()}),
    fun((KQK) -> {ok, KQK} | {error, snag:snag()})
) -> fun((binary()) -> {ok, KQK} | {error, snag:snag()}).
...
-spec flag_constraint(flag(KQG), fun((KQG) -> {ok, KQG} | {error, snag:snag()})) -> flag(KQG).
...
-spec flag_help(flag(KQZ), binary()) -> flag(KQZ).
...
-spec flag_default(flag(KRC), KRC) -> flag(KRC).
...
-spec flag(
    flag(KNN),
    fun((fun((flags()) -> {ok, KNN} | {error, snag:snag()})) -> command(KNQ))
) -> command(KNQ).
flag(Flag, F) ->
    Cmd = F(
        fun(_capture) ->
...
-spec fold(flags(), KRF, fun((KRF, binary(), flag_entry()) -> KRF)) -> KRF.
fold(Flags, Acc, F) ->
...
-spec command(fun((named_args(), list(binary()), flags()) -> KMX)) -> command(KMX).
command(Runner) ->
...
-spec get_value(
    flags(),
    binary(),
    fun((flag_entry()) -> {ok, KRS} | {error, snag:snag()})
) -> {ok, KRS} | {error, snag:snag()}.
...
-spec get_flag(flags(), flag(KRV)) -> {ok, KRV} | {error, snag:snag()}.
get_flag(Flags, Flag) ->
...
-spec do_update_at(
    command_node(KSO),
    list(binary()),
    fun((command_node(KSO)) -> command_node(KSO))
) -> command_node(KSO).
...
-spec update_at(
    glint(KSI),
    list(binary()),
    fun((command_node(KSI)) -> command_node(KSI))
) -> glint(KSI).
...
-spec path_help(glint(KMH), list(binary()), binary()) -> glint(KMH).
...
-spec add(glint(KMO), list(binary()), command(KMO)) -> glint(KMO).
...
-spec group_flag(glint(KNT), list(binary()), flag(any())) -> glint(KNT).
...
?DOC(false).
-spec execute(glint(KNZ), list(binary())) -> {ok, out(KNZ)} | {error, binary()}.
execute(Glint, Args) ->
    Help_flag = <<"--"/utf8,
        (erlang:element(
            2,
...
-spec run_and_handle(glint(KOY), list(binary()), fun((KOY) -> any())) -> nil.
