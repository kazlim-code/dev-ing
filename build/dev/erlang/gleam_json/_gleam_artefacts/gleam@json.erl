...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(EHN)) -> {ok, EHN} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(EHH)) -> {ok, EHH} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(EHD)) -> {ok, EHD} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(EHT), fun((EHT) -> json())) -> json().
...
-spec array(list(EHX), fun((EHX) -> json())) -> json().
