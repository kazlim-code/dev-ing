...
-type message(MGK) :: {internal, internal_message()} | {user, MGK}.

-type loop_message(MGL) :: {packet, bitstring()} | {custom, MGL}.

-type loop_state(MGM, MGN) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(MGN)),
        glisten@transport:transport(),
        MGM}.

-type connection(MGO) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(MGO))}.

-type next(MGP, MGQ) :: {continue,
        MGP,
        gleam@option:option(gleam@erlang@process:selector(MGQ))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(MGR, MGS) :: {handler,
        glisten@socket:socket(),
        fun((MGR, loop_message(MGS), connection(MGS)) -> next(MGR, loop_message(MGS))),
        fun((connection(MGS)) -> {MGR,
            gleam@option:option(gleam@erlang@process:selector(MGS))}),
        gleam@option:option(fun((MGR) -> nil)),
        glisten@transport:transport()}.

-file("src/glisten/internal/handler.gleam", 65).
?DOC(false).
-spec continue(MHF) -> next(MHF, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten/internal/handler.gleam", 69).
?DOC(false).
-spec with_selector(next(MHJ, MHK), gleam@erlang@process:selector(MHK)) -> next(MHJ, MHK).
...
?DOC(false).
-spec start(handler(any(), MHZ)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(MHZ)))} |
