...
-type message(NAY) :: {packet, bitstring()} | {user, NAY}.
...
-type connection(NAZ) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(NAZ))}.

-opaque next(NBA, NBB) :: {continue,
        NBA,
        gleam@option:option(gleam@erlang@process:selector(NBB))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(NBC, NBD) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(NBD)) -> {NBC,
            gleam@option:option(gleam@erlang@process:selector(NBD))}),
        fun((NBC, message(NBD), connection(NBD)) -> next(NBC, message(NBD))),
        gleam@option:option(fun((NBC) -> nil)),
        integer(),
        boolean(),
        boolean(),
...
-spec continue(NCD) -> next(NCD, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten.gleam", 151).
-spec with_selector(next(NCH, NCI), gleam@erlang@process:selector(NCI)) -> next(NCH, NCI).
...
?DOC(false).
-spec convert_next(next(NCW, NCX)) -> glisten@internal@handler:next(NCW, NCX).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(NDC, NDD), fun((NDD) -> NDG)) -> next(NDC, NDG).
...
-spec map_user_selector(gleam@erlang@process:selector(message(NDJ))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(NDJ)).
...
-spec convert_loop(
    fun((NDO, message(NDP), connection(NDP)) -> next(NDO, message(NDP)))
) -> fun((NDO, glisten@internal@handler:loop_message(NDP), glisten@internal@handler:connection(NDP)) -> glisten@internal@handler:next(NDO, glisten@internal@handler:loop_message(NDP))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(NDU)) -> {NDW,
        gleam@option:option(gleam@erlang@process:selector(NDU))})
) -> fun((glisten@internal@handler:connection(NDU)) -> {NDW,
    gleam@option:option(gleam@erlang@process:selector(NDU))}).
...
-spec new(
    fun((connection(NEC)) -> {NEE,
        gleam@option:option(gleam@erlang@process:selector(NEC))}),
    fun((NEE, message(NEC), connection(NEC)) -> next(NEE, message(NEC)))
) -> builder(NEE, NEC).
new(On_init, Loop) ->
...
-spec with_close(builder(NEL, NEM), fun((NEL) -> nil)) -> builder(NEL, NEM).
...
-spec with_pool_size(builder(NER, NES), integer()) -> builder(NER, NES).
...
?DOC(false).
-spec with_http2(builder(NEX, NEY)) -> builder(NEX, NEY).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_ipv6(builder(NFJ, NFK)) -> builder(NFJ, NFK).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(NFP, NFQ), binary(), binary()) -> builder(NFP, NFQ).
...
-spec bind(builder(NFD, NFE), binary()) -> builder(NFD, NFE).
