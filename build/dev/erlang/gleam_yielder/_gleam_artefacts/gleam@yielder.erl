...
-type action(IRS) :: stop | {continue, IRS, fun(() -> action(IRS))}.

-opaque yielder(IRT) :: {yielder, fun(() -> action(IRT))}.

-type step(IRU, IRV) :: {next, IRU, IRV} | done.

-type chunk(IRW, IRX) :: {another_by,
        list(IRW),
        IRX,
        IRW,
        fun(() -> action(IRW))} |
...
-type sized_chunk(IRY) :: {another, list(IRY), fun(() -> action(IRY))} |
    {last, list(IRY)} |
    no_more.
...
-spec unfold_loop(ISG, fun((ISG) -> step(ISH, ISG))) -> fun(() -> action(ISH)).
...
-spec unfold(ISB, fun((ISB) -> step(ISC, ISB))) -> yielder(ISC).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> ISL)) -> yielder(ISL).
repeatedly(F) ->
...
-spec from_list(list(ISP)) -> yielder(ISP).
from_list(List) ->
...
-spec transform(yielder(ISZ), ITB, fun((ITB, ISZ) -> step(ITC, ITB))) -> yielder(ITC).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 204).
-spec fold_loop(fun(() -> action(ITJ)), fun((ITL, ITJ) -> ITL), ITL) -> ITL.
...
-spec fold(yielder(ITG), ITI, fun((ITI, ITG) -> ITI)) -> ITI.
...
-spec to_list(yielder(ITO)) -> list(ITO).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(ITR)) -> step(ITR, yielder(ITR)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(ITZ)), integer()) -> fun(() -> action(ITZ)).
...
-spec take(yielder(ITW), integer()) -> yielder(ITW).
...
-spec drop_loop(fun(() -> action(IUF)), integer()) -> action(IUF).
...
-spec map(yielder(IUI), fun((IUI) -> IUK)) -> yielder(IUK).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(IUQ), yielder(IUS), fun((IUQ, IUS) -> IUU)) -> yielder(IUU).
...
-spec append_loop(fun(() -> action(IVG)), fun(() -> action(IVG))) -> action(IVG).
...
-spec append(yielder(IVC), yielder(IVC)) -> yielder(IVC).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(IVO)))) -> action(IVO).
...
-spec flatten(yielder(yielder(IVK))) -> yielder(IVK).
flatten(Yielder) ->
...
-spec concat(list(yielder(IVS))) -> yielder(IVS).
concat(Yielders) ->
...
-spec flat_map(yielder(IVW), fun((IVW) -> yielder(IVY))) -> yielder(IVY).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(IWE)), fun((IWE) -> boolean())) -> action(IWE).
...
-spec filter(yielder(IWB), fun((IWB) -> boolean())) -> yielder(IWB).
...
-spec filter_map(yielder(IWH), fun((IWH) -> {ok, IWJ} | {error, any()})) -> yielder(IWJ).
...
-spec cycle(yielder(IWV)) -> yielder(IWV).
cycle(Yielder) ->
    _pipe = repeat(Yielder),
    flatten(_pipe).

-file("src/gleam/yielder.gleam", 709).
-spec find_loop(fun(() -> action(IXD)), fun((IXD) -> boolean())) -> {ok, IXD} |
    {error, nil}.
...
-spec find(yielder(IWZ), fun((IWZ) -> boolean())) -> {ok, IWZ} | {error, nil}.
...
-spec find_map(yielder(IXH), fun((IXH) -> {ok, IXJ} | {error, any()})) -> {ok,
        IXJ} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(IYA)), integer()) -> fun(() -> action({IYA,
    integer()})).
...
-spec index(yielder(IXX)) -> yielder({IXX, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(IYD, fun((IYD) -> IYD)) -> yielder(IYD).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src/gleam/yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(IYI)), fun((IYI) -> boolean())) -> fun(() -> action(IYI)).
...
-spec take_while(yielder(IYF), fun((IYF) -> boolean())) -> yielder(IYF).
...
-spec drop_while_loop(fun(() -> action(IYO)), fun((IYO) -> boolean())) -> action(IYO).
...
-spec drop_while(yielder(IYL), fun((IYL) -> boolean())) -> yielder(IYL).
...
-spec scan_loop(fun(() -> action(IYV)), fun((IYX, IYV) -> IYX), IYX) -> fun(() -> action(IYX)).
...
-spec scan(yielder(IYR), IYT, fun((IYT, IYR) -> IYT)) -> yielder(IYT).
...
-spec zip_loop(fun(() -> action(IZE)), fun(() -> action(IZG))) -> fun(() -> action({IZE,
    IZG})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(IYZ), yielder(IZB)) -> yielder({IYZ, IZB}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(IZT)), fun((IZT) -> IZV), IZV, list(IZT)) -> chunk(IZT, IZV).
...
-spec chunk_loop(fun(() -> action(IZO)), fun((IZO) -> IZQ), IZQ, IZO) -> action(list(IZO)).
...
-spec chunk(yielder(IZJ), fun((IZJ) -> any())) -> yielder(list(IZJ)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(JAH)), integer(), list(JAH)) -> sized_chunk(JAH).
...
-spec sized_chunk_loop(fun(() -> action(JAD)), integer()) -> fun(() -> action(list(JAD))).
...
-spec sized_chunk(yielder(IZZ), integer()) -> yielder(list(IZZ)).
...
-spec intersperse_loop(fun(() -> action(JAO)), JAO) -> action(JAO).
...
-spec intersperse(yielder(JAL), JAL) -> yielder(JAL).
...
-spec any_loop(fun(() -> action(JAT)), fun((JAT) -> boolean())) -> boolean().
...
-spec any(yielder(JAR), fun((JAR) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1228).
-spec all_loop(fun(() -> action(JAX)), fun((JAX) -> boolean())) -> boolean().
...
-spec all(yielder(JAV), fun((JAV) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1273).
-spec update_group_with(JBN) -> fun((gleam@option:option(list(JBN))) -> list(JBN)).
...
-spec group_updater(fun((JBF) -> JBG)) -> fun((gleam@dict:dict(JBG, list(JBF)), JBF) -> gleam@dict:dict(JBG, list(JBF))).
group_updater(F) ->
...
-spec group(yielder(JAZ), fun((JAZ) -> JBB)) -> gleam@dict:dict(JBB, list(JAZ)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(JBR), fun((JBR, JBR) -> JBR)) -> {ok, JBR} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(JBV)) -> {ok, JBV} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(JCD) -> yielder(JCD).
single(Elem) ->
    once(fun() -> Elem end).

-file("src/gleam/yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(JCJ)), fun(() -> action(JCJ))) -> action(JCJ).
...
-spec interleave(yielder(JCF), yielder(JCF)) -> yielder(JCF).
...
-spec fold_until_loop(
    fun(() -> action(JCR)),
    fun((JCT, JCR) -> gleam@list:continue_or_stop(JCT)),
    JCT
) -> JCT.
...
-spec fold_until(
    yielder(JCN),
    JCP,
    fun((JCP, JCN) -> gleam@list:continue_or_stop(JCP))
) -> JCP.
...
-spec try_fold(yielder(JCV), JCX, fun((JCX, JCV) -> {ok, JCX} | {error, JCY})) -> {ok,
        JCX} |
    {error, JCY}.
...
-spec first(yielder(JDL)) -> {ok, JDL} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(JDP), integer()) -> {ok, JDP} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(JDX), fun((JDX) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(JEA, fun(() -> yielder(JEA))) -> yielder(JEA).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(JED), JED) -> yielder(JED).
