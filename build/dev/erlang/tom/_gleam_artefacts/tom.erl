...
-spec push_key({ok, ABMW} | {error, get_error()}, binary()) -> {ok, ABMW} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {ABPB, list(binary())}} |
        {error, parse_error()})
) -> {ok, {ABPB, list(binary())}} | {error, parse_error()}.
...
-spec do(
    {ok, {ABPM, list(binary())}} | {error, parse_error()},
    fun((ABPM, list(binary())) -> {ok, ABPP} | {error, parse_error()})
) -> {ok, ABPP} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {ABPU, list(binary())}} |
        {error, parse_error()})
) -> {ok, {ABPU, list(binary())}} | {error, parse_error()}.
