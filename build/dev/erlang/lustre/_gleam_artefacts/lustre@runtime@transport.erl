...
-type client_message(SAF) :: {mount,
        ...
        lustre@vdom@vnode:element(SAF)} |
    {reconcile, integer(), lustre@vdom@patch:patch(SAF)} |
...
?DOC(false).
-spec reconcile(lustre@vdom@patch:patch(SAO)) -> client_message(SAO).
reconcile(Patch) ->
    {reconcile, 1, Patch}.
