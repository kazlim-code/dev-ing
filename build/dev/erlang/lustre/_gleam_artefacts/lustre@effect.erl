...
-opaque effect(PJF) :: {effect,
        list(fun((actions(PJF)) -> nil)),
        list(fun((actions(PJF)) -> nil)),
        list(fun((actions(PJF)) -> nil))}.

-type actions(PJG) :: {actions,
        fun((PJG) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(PJG)) -> nil),
...
-spec do_comap_select(
    actions(PKQ),
    gleam@erlang@process:selector(PKS),
    fun((PKS) -> PKQ)
) -> nil.
...
-spec do_comap_actions(actions(PKM), fun((PKO) -> PKM)) -> actions(PKO).
...
-spec do_map(list(fun((actions(PKG)) -> nil)), fun((PKG) -> PKJ)) -> list(fun((actions(PKJ)) -> nil)).
do_map(Effects, F) ->
    gleam@list:map(
        Effects,
        fun(Effect) ->
...
-spec map(effect(PKC), fun((PKC) -> PKE)) -> effect(PKE).
map(Effect, F) ->
    {effect,
...
-spec perform(
    ...
    fun((PKU) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(PKU)) -> nil),
    fun(() -> gleam@dynamic:dynamic_()),
    fun((binary(), gleam@json:json()) -> nil)
) -> nil.
...
-spec from(fun((fun((PJJ) -> nil)) -> nil)) -> effect(PJJ).
from(Effect) ->
    Task = fun(Actions) ->
...
-spec before_paint(fun((fun((PJL) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(PJL).
before_paint(Effect) ->
    Task = fun(Actions) ->
...
-spec after_paint(fun((fun((PJN) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(PJN).
after_paint(Effect) ->
    Task = fun(Actions) ->
        Root = (erlang:element(5, Actions))(),
        Dispatch = erlang:element(2, Actions),
...
?DOC(false).
-spec select(
    fun((fun((PJR) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(PJR))
) -> effect(PJR).
select(Sel) ->
    Task = fun(Actions) ->
...
-spec batch(list(effect(PJY))) -> effect(PJY).
batch(Effects) ->
    gleam@list:fold(
        Effects,
        {effect, [], [], []},
