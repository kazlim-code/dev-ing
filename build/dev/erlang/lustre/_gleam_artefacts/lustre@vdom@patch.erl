...
-type diff(RVD) :: {diff, patch(RVD), lustre@vdom@events:events(RVD)}.

-type patch(RVE) :: {patch,
        integer(),
        integer(),
        list(change(RVE)),
        list(patch(RVE))}.

-type change(RVF) :: {replace_text, integer(), binary()} |
    {replace_inner_html, integer(), binary()} |
    {update,
        integer(),
        list(lustre@vdom@vattr:attribute(RVF)),
        list(lustre@vdom@vattr:attribute(RVF))} |
    {move, integer(), binary(), integer()} |
    {replace, integer(), integer(), lustre@vdom@vnode:element(RVF)} |
    {remove, integer(), integer()} |
    {insert, integer(), list(lustre@vdom@vnode:element(RVF)), integer()}.

-file("src/lustre/vdom/patch.gleam", 73).
?DOC(false).
-spec new(integer(), integer(), list(change(RVG)), list(patch(RVG))) -> patch(RVG).
...
?DOC(false).
-spec add_child(patch(RWJ), patch(RWJ)) -> patch(RWJ).
...
?DOC(false).
-spec update(
    list(lustre@vdom@vattr:attribute(RVQ)),
    list(lustre@vdom@vattr:attribute(RVQ))
) -> change(RVQ).
...
?DOC(false).
-spec replace(integer(), lustre@vdom@vnode:element(RWA)) -> change(RWA).
replace(Index, With) ->
    {replace, 5, Index, With}.

-file("src/lustre/vdom/patch.gleam", 123).
?DOC(false).
-spec insert(list(lustre@vdom@vnode:element(RWD)), integer()) -> change(RWD).
