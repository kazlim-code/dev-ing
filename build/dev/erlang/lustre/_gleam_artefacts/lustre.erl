...
-opaque app(UFE, UFF, UFG) :: {app,
        fun((UFE) -> {UFF, lustre@effect:effect(UFG)}),
        fun((UFF, UFG) -> {UFF, lustre@effect:effect(UFG)}),
        fun((UFF) -> lustre@vdom@vnode:element(UFG)),
        lustre@component:config(UFG)}.
...
-type runtime(UFH) :: any() | {gleam_phantom, UFH}.
...
-spec component(
    fun((UGG) -> {UGH, lustre@effect:effect(UGI)}),
    fun((UGH, UGI) -> {UGH, lustre@effect:effect(UGI)}),
    fun((UGH) -> lustre@vdom@vnode:element(UGI)),
    list(lustre@component:option(UGI))
) -> app(UGG, UGH, UGI).
...
-spec application(
    fun((UFX) -> {UFY, lustre@effect:effect(UFZ)}),
    fun((UFY, UFZ) -> {UFY, lustre@effect:effect(UFZ)}),
    fun((UFY) -> lustre@vdom@vnode:element(UFZ))
) -> app(UFX, UFY, UFZ).
...
-spec element(lustre@vdom@vnode:element(UFK)) -> app(any(), nil, UFK).
element(View) ->
    application(
...
-spec simple(
    fun((UFQ) -> UFR),
    fun((UFR, UFS) -> UFR),
    fun((UFR) -> lustre@vdom@vnode:element(UFS))
) -> app(UFQ, UFR, UFS).
...
-spec do_start(app(UHA, any(), UHC), binary(), UHA) -> {ok, runtime(UHC)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.
...
-spec send(runtime(UHZ), lustre@runtime@server@runtime:message(UHZ)) -> nil.
...
-spec dispatch(UIC) -> lustre@runtime@server@runtime:message(UIC).
dispatch(Msg) ->
...
-spec start(app(UGR, any(), UGT), binary(), UGR) -> {ok, runtime(UGT)} |
    {error, error()}.
...
-spec start_server_component(app(UHJ, any(), UHL), UHJ) -> {ok, runtime(UHL)} |
    {error, error()}.
