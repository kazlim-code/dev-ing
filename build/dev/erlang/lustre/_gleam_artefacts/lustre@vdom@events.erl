...
-opaque events(PZP) :: {events,
        lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(PZP))),
        list(binary()),
        list(binary())}.
...
?DOC(false).
-spec tick(events(PZV)) -> events(PZV).
tick(Events) ->
...
-spec do_remove_event(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QAR))),
...
?DOC(false).
-spec remove_event(events(QAO), lustre@vdom@path:path(), binary()) -> events(QAO).
...
-spec remove_attributes(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QDD))),
    lustre@vdom@path:path(),
...
?DOC(false).
-spec handle(events(QDZ), binary(), binary(), gleam@dynamic:dynamic_()) -> {events(QDZ),
    {ok, lustre@vdom@vattr:handler(QDZ)} |
...
?DOC(false).
-spec do_add_event(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QAD))),
    fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QAD))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QAD))).
...
-spec add_event(
    ...
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(PZY))
) -> events(PZY).
...
?DOC(false).
-spec add_attributes(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QBO))),
    fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
    lustre@vdom@path:path(),
...
?DOC(false).
-spec is_reference_equal(QEL, QEL) -> boolean().
...
-spec do_remove_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QDO))),
    lustre@vdom@path:path(),
    integer(),
...
-spec do_remove_child(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QCT))),
    lustre@vdom@path:path(),
    integer(),
...
-spec do_add_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QCE))),
    fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
    lustre@vdom@path:path(),
    integer(),
...
?DOC(false).
-spec do_add_child(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QBE))),
    fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
    lustre@vdom@path:path(),
    integer(),
    lustre@vdom@vnode:element(QBE)
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(QBE))).
...
?DOC(false).
-spec from_node(lustre@vdom@vnode:element(PZS)) -> events(PZS).
from_node(Root) ->
...
-spec add_children(
    ...
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(QBZ))
) -> events(QBZ).
