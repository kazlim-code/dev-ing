...
-type attribute(OLJ) :: {attribute, integer(), binary(), binary()} |
    {property, integer(), binary(), gleam@json:json()} |
    {event,
        integer(),
        binary(),
        gleam@dynamic@decode:decoder(handler(OLJ)),
        list(binary()),
        event_behaviour(),
        event_behaviour(),
        boolean(),
...
-type handler(OLK) :: {handler, boolean(), boolean(), OLK}.
...
?DOC(false).
-spec merge(list(attribute(OLZ)), list(attribute(OLZ))) -> list(attribute(OLZ)).
...
?DOC(false).
-spec compare(attribute(OMG), attribute(OMG)) -> gleam@order:order().
compare(A, B) ->
    gleam@string:compare(erlang:element(3, A), erlang:element(3, B)).

-file("src/lustre/vdom/vattr.gleam", 94).
?DOC(false).
-spec prepare(list(attribute(OLU))) -> list(attribute(OLU)).
prepare(Attributes) ->
    case Attributes of
        [] ->
            Attributes;
...
-spec event(
    binary(),
    gleam@dynamic@decode:decoder(handler(OLP)),
    list(binary()),
    event_behaviour(),
    event_behaviour(),
    boolean(),
