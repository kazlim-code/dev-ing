...
-type state(SPI, SPJ) :: {state,
        gleam@erlang@process:subject(message(SPJ)),
        gleam@erlang@process:selector(message(SPJ)),
        gleam@erlang@process:selector(message(SPJ)),
        SPI,
        fun((SPI, SPJ) -> {SPI, lustre@effect:effect(SPJ)}),
        fun((SPI) -> lustre@vdom@vnode:element(SPJ)),
        config(SPJ),
        lustre@vdom@vnode:element(SPJ),
        lustre@vdom@events:events(SPJ),
        gleam@dict:dict(binary(), gleam@json:json()),
        gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(SPJ)), gleam@erlang@process:monitor()),
        gleam@set:set(fun((lustre@runtime@transport:client_message(SPJ)) -> nil))}.

-type config(SPK) :: {config,
        boolean(),
        boolean(),
        gleam@dict:dict(binary(), fun((binary()) -> {ok, SPK} | {error, nil})),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(SPK)),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(SPK))}.

-type message(SPL) :: {client_dispatched_message,
        lustre@runtime@transport:server_message()} |
    {client_registered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(SPL))} |
    {client_deregistered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(SPL))} |
    {client_registered_callback,
        fun((lustre@runtime@transport:client_message(SPL)) -> nil)} |
    {client_deregistered_callback,
        fun((lustre@runtime@transport:client_message(SPL)) -> nil)} |
    {effect_added_selector, gleam@erlang@process:selector(message(SPL))} |
    {effect_dispatched_message, SPL} |
...
-spec handle_attribute_change(
    gleam@dict:dict(binary(), fun((binary()) -> {ok, SQP} | {error, nil})),
    binary(),
    binary()
) -> {ok, SQP} | {error, nil}.
...
?DOC(false).
-spec handle_effect(
    gleam@erlang@process:subject(message(SRC)),
    lustre@effect:effect(SRC)
) -> nil.
...
?DOC(false).
-spec broadcast(
    gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(SRG)), gleam@erlang@process:monitor()),
    gleam@set:set(fun((lustre@runtime@transport:client_message(SRG)) -> nil)),
    lustre@runtime@transport:client_message(SRG)
) -> nil.
...
?DOC(false).
-spec loop(state(SPZ, SQA), message(SQA)) -> gleam@otp@actor:next(state(SPZ, SQA), message(SQA)).
loop(State, Message) ->
    case Message of
...
?DOC(false).
-spec start(
    {SPP, lustre@effect:effect(SPQ)},
    fun((SPP, SPQ) -> {SPP, lustre@effect:effect(SPQ)}),
    fun((SPP) -> lustre@vdom@vnode:element(SPQ)),
    config(SPQ)
) -> {ok, gleam@erlang@process:subject(message(SPQ))} |
