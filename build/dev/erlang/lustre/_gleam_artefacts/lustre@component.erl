...
-opaque config(TVD) :: {config,
        boolean(),
        boolean(),
        boolean(),
        list({binary(), fun((binary()) -> {ok, TVD} | {error, nil})}),
        list({binary(), gleam@dynamic@decode:decoder(TVD)}),
        list({binary(), gleam@dynamic@decode:decoder(TVD)}),
        boolean(),
        gleam@option:option(fun((binary()) -> TVD)),
        gleam@option:option(TVD),
        gleam@option:option(fun((binary()) -> TVD))}.

-opaque option(TVE) :: {option, fun((config(TVE)) -> config(TVE))}.

-file("src/lustre/component.gleam", 133).
?DOC(false).
-spec new(list(option(TVF))) -> config(TVF).
new(Options) ->
...
-spec on_attribute_change(binary(), fun((binary()) -> {ok, TVJ} | {error, nil})) -> option(TVJ).
...
-spec on_property_change(binary(), gleam@dynamic@decode:decoder(TVN)) -> option(TVN).
...
-spec on_context_change(binary(), gleam@dynamic@decode:decoder(TVQ)) -> option(TVQ).
...
-spec on_form_autofill(fun((binary()) -> TVV)) -> option(TVV).
...
-spec on_form_reset(TVX) -> option(TVX).
on_form_reset(Msg) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_form_restore(fun((binary()) -> TVZ)) -> option(TVZ).
...
?DOC(false).
-spec to_server_component_config(config(TWH)) -> lustre@runtime@server@runtime:config(TWH).
