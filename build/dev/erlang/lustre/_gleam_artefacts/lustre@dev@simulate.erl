...
-opaque app(UZR, UZS, UZT) :: {app,
        fun((UZR) -> {UZS, lustre@effect:effect(UZT)}),
        fun((UZS, UZT) -> {UZS, lustre@effect:effect(UZT)}),
        fun((UZS) -> lustre@vdom@vnode:element(UZT))}.

-opaque simulation(UZU, UZV) :: {simulation,
        fun((UZU, UZV) -> {UZU, lustre@effect:effect(UZV)}),
        fun((UZU) -> lustre@vdom@vnode:element(UZV)),
        list(event(UZV)),
        UZU,
        lustre@vdom@vnode:element(UZV)}.

-type event(UZW) :: {dispatch, UZW} |
...
-spec simple(
    fun((UZX) -> UZY),
    fun((UZY, UZZ) -> UZY),
    fun((UZY) -> lustre@vdom@vnode:element(UZZ))
) -> app(UZX, UZY, UZZ).
...
-spec application(
    fun((VAE) -> {VAF, lustre@effect:effect(VAG)}),
    fun((VAF, VAG) -> {VAF, lustre@effect:effect(VAG)}),
    fun((VAF) -> lustre@vdom@vnode:element(VAG))
) -> app(VAE, VAF, VAG).
...
-spec start(app(VAN, VAO, VAP), VAN) -> simulation(VAO, VAP).
start(App, Args) ->
...
-spec message(simulation(VAV, VAW), VAW) -> simulation(VAV, VAW).
...
-spec problem(simulation(VCB, VCC), binary(), binary()) -> simulation(VCB, VCC).
...
-spec model(simulation(VCH, any())) -> VCH.
model(Simulation) ->
    erlang:element(5, Simulation).
...
-spec view(simulation(any(), VCM)) -> lustre@vdom@vnode:element(VCM).
view(Simulation) ->
    erlang:element(6, Simulation).
...
-spec history(simulation(any(), VCR)) -> list(event(VCR)).
history(Simulation) ->
...
-spec click(simulation(VBI, VBJ), lustre@dev@query:'query'()) -> simulation(VBI, VBJ).
...
-spec input(simulation(VBO, VBP), lustre@dev@query:'query'(), binary()) -> simulation(VBO, VBP).
