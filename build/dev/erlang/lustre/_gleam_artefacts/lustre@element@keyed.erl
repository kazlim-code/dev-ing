...
-spec do_extract_keyed_children(
    list({binary(), lustre@vdom@vnode:element(VKP)}),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(VKP)),
    list(lustre@vdom@vnode:element(VKP))
) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(VKP)),
    list(lustre@vdom@vnode:element(VKP))}.
...
-spec extract_keyed_children(list({binary(), lustre@vdom@vnode:element(VKH)})) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(VKH)),
    list(lustre@vdom@vnode:element(VKH))}.
...
-spec element(
    binary(),
    list(lustre@vdom@vattr:attribute(VIN)),
    list({binary(), lustre@vdom@vnode:element(VIN)})
) -> lustre@vdom@vnode:element(VIN).
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(VIT)),
    list({binary(), lustre@vdom@vnode:element(VIT)})
) -> lustre@vdom@vnode:element(VIT).
...
-spec fragment(list({binary(), lustre@vdom@vnode:element(VIZ)})) -> lustre@vdom@vnode:element(VIZ).
fragment(Children) ->
    {Keyed_children, Children@1} = extract_keyed_children(Children),
...
-spec ul(
    list(lustre@vdom@vattr:attribute(VJD)),
    list({binary(), lustre@vdom@vnode:element(VJD)})
) -> lustre@vdom@vnode:element(VJD).
ul(Attributes, Children) ->
    element(<<"ul"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 148).
-spec ol(
    list(lustre@vdom@vattr:attribute(VJJ)),
    list({binary(), lustre@vdom@vnode:element(VJJ)})
) -> lustre@vdom@vnode:element(VJJ).
ol(Attributes, Children) ->
    element(<<"ol"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 155).
-spec 'div'(
    list(lustre@vdom@vattr:attribute(VJP)),
    list({binary(), lustre@vdom@vnode:element(VJP)})
) -> lustre@vdom@vnode:element(VJP).
'div'(Attributes, Children) ->
    element(<<"div"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 162).
-spec tbody(
    list(lustre@vdom@vattr:attribute(VJV)),
    list({binary(), lustre@vdom@vnode:element(VJV)})
) -> lustre@vdom@vnode:element(VJV).
tbody(Attributes, Children) ->
    element(<<"tbody"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 169).
-spec dl(
    list(lustre@vdom@vattr:attribute(VKB)),
    list({binary(), lustre@vdom@vnode:element(VKB)})
) -> lustre@vdom@vnode:element(VKB).
