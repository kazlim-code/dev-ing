...
-spec fragment(list(lustre@vdom@vnode:element(QKQ))) -> lustre@vdom@vnode:element(QKQ).
fragment(Children) ->
...
-spec unsafe_raw_html(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(QKU)),
    binary()
) -> lustre@vdom@vnode:element(QKU).
...
-spec map(lustre@vdom@vnode:element(QKY), fun((QKY) -> QLA)) -> lustre@vdom@vnode:element(QLA).
map(Element, F) ->
