...
-spec handler(WCK, boolean(), boolean()) -> lustre@vdom@vattr:handler(WCK).
...
-spec on(binary(), gleam@dynamic@decode:decoder(WCD)) -> lustre@vdom@vattr:attribute(WCD).
on(Name, Handler) ->
...
-spec advanced(
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(WCG))
) -> lustre@vdom@vattr:attribute(WCG).
...
-spec prevent_default(lustre@vdom@vattr:attribute(WCM)) -> lustre@vdom@vattr:attribute(WCM).
...
-spec stop_propagation(lustre@vdom@vattr:attribute(WCP)) -> lustre@vdom@vattr:attribute(WCP).
...
-spec debounce(lustre@vdom@vattr:attribute(WCS), integer()) -> lustre@vdom@vattr:attribute(WCS).
...
-spec throttle(lustre@vdom@vattr:attribute(WCV), integer()) -> lustre@vdom@vattr:attribute(WCV).
...
-spec on_keypress(fun((binary()) -> WDM)) -> lustre@vdom@vattr:attribute(WDM).
on_keypress(Msg) ->
    on(
        <<"keypress"/utf8>>,
        begin
...
-spec on_keydown(fun((binary()) -> WDO)) -> lustre@vdom@vattr:attribute(WDO).
on_keydown(Msg) ->
    on(
        <<"keydown"/utf8>>,
        begin
...
-spec on_keyup(fun((binary()) -> WDQ)) -> lustre@vdom@vattr:attribute(WDQ).
on_keyup(Msg) ->
    on(
        <<"keyup"/utf8>>,
        begin
...
-spec on_input(fun((binary()) -> WDS)) -> lustre@vdom@vattr:attribute(WDS).
on_input(Msg) ->
    on(
        <<"input"/utf8>>,
        begin
...
-spec on_change(fun((binary()) -> WDU)) -> lustre@vdom@vattr:attribute(WDU).
on_change(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_check(fun((boolean()) -> WDW)) -> lustre@vdom@vattr:attribute(WDW).
on_check(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_submit(fun((list({binary(), binary()})) -> WDZ)) -> lustre@vdom@vattr:attribute(WDZ).
on_submit(Msg) ->
    _pipe@2 = on(
        <<"submit"/utf8>>,
        begin
...
-spec on_focus(WED) -> lustre@vdom@vattr:attribute(WED).
on_focus(Msg) ->
    on(<<"focus"/utf8>>, gleam@dynamic@decode:success(Msg)).

-file("src/lustre/event.gleam", 346).
-spec on_blur(WEF) -> lustre@vdom@vattr:attribute(WEF).
on_blur(Msg) ->
