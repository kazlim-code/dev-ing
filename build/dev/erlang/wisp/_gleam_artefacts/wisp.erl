...
-spec method_override(gleam@http@request:request(AEIT)) -> gleam@http@request:request(AEIT).
...
-spec fn_with_bad_request_error(fun((AEJU) -> {ok, AEJV} | {error, any()})) -> fun((AEJU) -> {ok,
        AEJV} |
    {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((AEJO, bitstring()) -> {ok, AEJO} |
        {error, gleam@http@response:response(body())}),
    AEJO
) -> {ok, {gleam@option:option(buffered_reader()), integer(), AEJO}} |
    {error, gleam@http@response:response(body())}.
...
-spec sort_keys(list({binary(), AEKM})) -> list({binary(), AEKM}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_400(
    {ok, AEKP} | {error, any()},
    fun((AEKP) -> gleam@http@response:response(body()))
) -> gleam@http@response:response(body()).
or_400(Result, Next) ->
    case Result of
        {ok, Value} ->
...
-spec or_500({ok, AEJG} | {error, any()}) -> {ok, AEJG} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
