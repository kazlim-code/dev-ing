...
-spec set_header(request(GGJ), binary(), binary()) -> request(GGJ).
...
-spec prepend_header(request(GGM), binary(), binary()) -> request(GGM).
...
-spec set_body(request(any()), GGR) -> request(GGR).
set_body(Req, Body) ->
...
-spec map(request(GGT), fun((GGT) -> GGV)) -> request(GGV).
...
-spec set_query(request(GHF), list({binary(), binary()})) -> request(GHF).
set_query(Req, Query) ->
    Pair = fun(T) ->
...
-spec set_method(request(GHJ), gleam@http:method()) -> request(GHJ).
...
-spec set_scheme(request(GHQ), gleam@http:scheme()) -> request(GHQ).
...
-spec set_host(request(GHT), binary()) -> request(GHT).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(GHW), integer()) -> request(GHW).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(GHZ), binary()) -> request(GHZ).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(GIC), binary(), binary()) -> request(GIC).
...
-spec remove_cookie(request(GIH), binary()) -> request(GIH).
