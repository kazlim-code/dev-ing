...
-type response(GQE) :: {response, integer(), list({binary(), binary()}), GQE}.
...
-spec set_header(response(GQT), binary(), binary()) -> response(GQT).
...
-spec prepend_header(response(GQW), binary(), binary()) -> response(GQW).
...
-spec set_body(response(any()), GRB) -> response(GRB).
...
-spec try_map(response(GQF), fun((GQF) -> {ok, GQH} | {error, GQI})) -> {ok,
        response(GQH)} |
    {error, GQI}.
...
-spec map(response(GRD), fun((GRD) -> GRF)) -> response(GRF).
...
-spec expire_cookie(response(GRN), binary(), gleam@http@cookie:attributes()) -> response(GRN).
