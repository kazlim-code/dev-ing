...
-spec map_response_body(
    fun((GUR) -> gleam@http@response:response(GUD)),
    fun((GUD) -> GUE)
) -> fun((GUR) -> gleam@http@response:response(GUE)).
...
-spec prepend_response_header(
    fun((GUV) -> gleam@http@response:response(GUX)),
    binary(),
    binary()
) -> fun((GUV) -> gleam@http@response:response(GUX)).
...
-spec ensure_post(gleam@http@request:request(GUI)) -> {ok,
        gleam@http@request:request(GUI)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(GWE)) -> GWM)) -> fun((gleam@http@request:request(GWE)) -> GWM).
