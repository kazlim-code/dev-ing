...
-type message(HHC) :: {message, HHC} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(HHD, HHE) :: {continue,
        HHD,
        gleam@option:option(gleam@erlang@process:selector(HHE))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(HHF, HHG) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        HHF,
        gleam@erlang@process:selector(message(HHG)),
        gleam@otp@system:debug_state(),
        fun((HHF, HHG) -> next(HHF, HHG))}.

-type started(HHH) :: {started, gleam@erlang@process:pid_(), HHH}.

-opaque initialised(HHI, HHJ, HHK) :: {initialised,
        HHI,
        gleam@option:option(gleam@erlang@process:selector(HHJ)),
        HHK}.

-opaque builder(HHL, HHM, HHN) :: {builder,
        fun((gleam@erlang@process:subject(HHM)) -> {ok,
                initialised(HHL, HHM, HHN)} |
            {error, binary()}),
        integer(),
        fun((HHL, HHM) -> next(HHL, HHM)),
        gleam@option:option(gleam@erlang@process:name(HHM))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(HHO) :: {ack, {ok, HHO} | {error, binary()}} |
...
-spec continue(HHT) -> next(HHT, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(HIF, HIG), gleam@erlang@process:selector(HIG)) -> next(HIF, HIG).
...
-spec initialised(HIM) -> initialised(HIM, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(HIR, any(), HIT),
    gleam@erlang@process:selector(HIX)
) -> initialised(HIR, HIX, HIT).
...
-spec returning(initialised(HJC, HJD, any()), HJI) -> initialised(HJC, HJD, HJI).
...
-spec new(HJM) -> builder(HJM, HJN, gleam@erlang@process:subject(HJN)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(HJS)) -> {ok, initialised(HJU, HJS, HJV)} |
        {error, binary()})
) -> builder(HJU, HJS, HJV).
...
-spec on_message(builder(HKE, HKF, HKG), fun((HKE, HKF) -> next(HKE, HKF))) -> builder(HKE, HKF, HKG).
...
-spec named(builder(HKP, HKQ, HKR), gleam@erlang@process:name(HKQ)) -> builder(HKP, HKQ, HKR).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(HLE))) -> gleam@erlang@process:selector(message(HLE)).
...
-spec receive_message(self(any(), HLA)) -> message(HLA).
...
-spec initialise_actor(
    builder(any(), any(), HLX),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, HLX} | {error, binary()})
...
-spec start(builder(any(), any(), HMK)) -> {ok, started(HMK)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(HMR), HMR) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(HMT),
    integer(),
    fun((gleam@erlang@process:subject(HMV)) -> HMT)
) -> HMV.
