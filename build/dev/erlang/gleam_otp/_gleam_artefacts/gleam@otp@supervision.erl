...
-type child_specification(IAH) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(IAH)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(IAI)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(IAI).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(IAN)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(IAN).
supervisor(Start) ->
...
-spec significant(child_specification(IAS), boolean()) -> child_specification(IAS).
...
-spec timeout(child_specification(IAV), integer()) -> child_specification(IAV).
timeout(Child, Ms) ->
...
-spec restart(child_specification(IAY), restart()) -> child_specification(IAY).
...
-spec map_data(child_specification(IBB), fun((IBB) -> IBD)) -> child_specification(IBD).
