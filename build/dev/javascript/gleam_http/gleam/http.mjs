...
function is_valid_tchar(ch) {
  if (ch === 33) {
    ...
    if ((ch$1 >= 0x30) && (ch$1 <= 0x39)) {
      ...
      let ch$2 = ch;
      if (
        ((ch$2 >= 0x41) && (ch$2 <= 0x5A)) || ((ch$2 >= 0x61) && (ch$2 <= 0x7A))
      ) {
        return true;
      } else {
        return false;
...
function do_is_valid_token(loop$bytes, loop$acc) {
  while (true) {
    ...
    let acc = loop$acc;
    if (acc && bytes.bitSize >= 8 && (bytes.bitSize - 8) % 8 === 0) {
...
function skip_whitespace(loop$data) {
  while (true) {
    ...
    if (data.bitSize >= 8) {
      ...
      } else if (data.byteAt(0) === 9 && (data.bitSize - 8) % 8 === 0) {
        let data$1 = bitArraySlice(data, 8);
        loop$data = data$1;
      } else {
        return data;
...
function parse_header_value(loop$data, loop$headers, loop$name, loop$value) {
  while (true) {
    ...
    if (size < 4) {
      ...
      if (data.byteAt(0) === 13 && data.bitSize >= 16 && data.byteAt(1) === 10) {
        if (data.bitSize >= 24) {
          if (
            data.byteAt(2) === 13 &&
            data.bitSize >= 32 &&
          ...
          } else if (data.byteAt(2) === 9 && (data.bitSize - 24) % 8 === 0) {
            let data$1 = bitArraySlice(data, 24);
            loop$data = data$1;
            loop$headers = headers;
            loop$name = name;
